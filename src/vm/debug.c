...
static int invokeInstruction(DictuVM *vm, const char* name, Chunk* chunk, int offset) {
    uint8_t argCount = chunk->code[offset + 1];
...
static int classInstruction(DictuVM *vm, const char* name, Chunk* chunk, int offset) {
