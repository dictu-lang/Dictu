...
static Value logObjUnsetPrefix(DictuVM *vm, int argCount, Value *args) {
    if (argCount != 0) {
        runtimeError(vm, "unsetPrefix() takes no arguments (%d given)", argCount);
        return EMPTY_VAL;
    }
...
char *logToString(DictuVM *vm, ObjAbstract *abstract, int *length) {
    UNUSED(abstract);

    int len = 5;
    char *logString = ALLOCATE(vm, char, len + 1);
    memcpy(logString, "<Log>", len);
    logString[len] = '\0';
    *length = len;
    return logString;
}
