...
static Value uuidGenerateNative(DictuVM *vm, int argCount, Value *args) {
    ...
    if (argCount != 0) {
        runtimeError(vm, "generate() takes no arguments (%d given)", argCount);
        return EMPTY_VAL;
    }
...
static Value uuidGenRandomNative(DictuVM *vm, int argCount, Value *args) {
    ...
    if (argCount != 0) {
        runtimeError(vm, "generateRandom() takes no arguments (%d given)", argCount);
        return EMPTY_VAL;
    }
...
static Value uuidGenTimeNative(DictuVM *vm, int argCount, Value *args) {
    ...
    if (argCount != 0) {
        runtimeError(vm, "generateTime() takes no arguments (%d given)", argCount);
        return EMPTY_VAL;
    }
